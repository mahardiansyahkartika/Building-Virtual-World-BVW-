...
public class LeapImageRetriever : MonoBehaviour {
	...
	protected bool isHandExist = false;
	...
	void Update () {
		...
		if (frame.Images.Count == 0) {
			...
			if (image_misses_ == IMAGE_WARNING_WAIT) {
				Debug.LogWarning ("Can't find any images. " + "Make sure you enabled 'Allow Images' in the Leap Motion Settings, " + "you are on tracking version 2.1+ and " + "your Leap Motion device is plugged in.");
			}
			return;
		}
		...
		if (frame.Hands.Count > 0) isHandExist = true;
		else isHandExist = false;
	...
	protected void EncodeDistortion() {
		...
		for (int i = 0; i < num_distortion_floats; ++i) {
			...
			if (dval > 1.0f || dval < 0.0f) {
				Debug.Log ("WARNING: got a distortion value outside my encoded range at pixel " + i + ": " + distortion_data_ [i]);
			}
