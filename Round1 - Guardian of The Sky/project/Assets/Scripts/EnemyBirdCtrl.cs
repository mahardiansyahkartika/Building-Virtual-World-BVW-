...
public class EnemyBirdCtrl : MonoBehaviour {
	public float forwardSpeed = 13;	//the maximum speed moving forward(when right behind the bird group)
	public GameObject characterA;	//the 'character A'
	...
	void FixedUpdate(){
		if (!isGetVictim && chaseTarget != null) {
			//control of speed(flying from front be slower, flying fron back be faster)
